    ...
    public class QuestInspector : Editor
    {
        ...
        protected void CreateRewardList(SerializedObject serializedObject, SerializedProperty elements)
        {
            ...
            this.m_RewardList.onAddCallback = (ReorderableList list) => {
                Type[] types = AppDomain.CurrentDomain.GetAssemblies().SelectMany(assembly => assembly.GetTypes()).Where(x => typeof(QuestReward).IsAssignableFrom(x) && !x.IsAbstract && !x.HasAttribute(typeof(ExcludeFromCreationAttribute))).ToArray();
        ...
        protected void CreateTaskList(SerializedObject serializedObject, SerializedProperty elements) {
            ...
            this.m_TaskList.onAddCallback = (ReorderableList list) => {
                Type[] types = AppDomain.CurrentDomain.GetAssemblies().SelectMany(assembly => assembly.GetTypes()).Where(x => typeof(QuestTask).IsAssignableFrom(x) && !x.IsAbstract && !x.HasAttribute(typeof(ExcludeFromCreationAttribute))).ToArray();
                types = types.OrderBy(x => x.BaseType.Name).ToArray();
                if (types.Length > 1) {
                    GenericMenu menu = new GenericMenu();
                    for (int i = 0; i < types.Length; i++) {
