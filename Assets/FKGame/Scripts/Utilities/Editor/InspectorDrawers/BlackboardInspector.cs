    ...
    public class BlackboardInspector : Editor
    {
        ...
        public override void OnInspectorGUI(){
            ...
            if (string.IsNullOrEmpty(this.m_VariableName))
            {
                ...
                GUI.Label(variableNameRect, LanguagesMacro.NEW_VARIABLE_NAME, variableNameOverlayStyle);
            }
            ...
            if (GUILayout.Button(EditorGUIUtility.IconContent("d_Toolbar Plus"), createAddNewDropDown, GUILayout.Width(35f)))
            {
                if (string.IsNullOrEmpty(this.m_VariableName))
                {
                    EditorUtility.DisplayDialog(LanguagesMacro.NEW_VARIABLE, LanguagesMacro.ENTER_A_VARIABLE_NAME, "OK");
                }else if (VariableNameExists(this.m_VariableName)){
                    EditorUtility.DisplayDialog(LanguagesMacro.NEW_VARIABLE, LanguagesMacro.EXISTED_VARIABLE_NAME, "OK");
                }
                else
                {
