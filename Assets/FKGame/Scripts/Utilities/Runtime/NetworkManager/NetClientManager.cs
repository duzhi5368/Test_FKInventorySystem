...
namespace FKGame
{
    ...
    {
        private INetwork s_network;
        private Dictionary<Type, INetPluginBase> plugins = new Dictionary<Type, INetPluginBase>();
        ...
        public void Init<TProtocol, TSocket>(ProtocolType protocolType = ProtocolType.Tcp) where TProtocol : INetwork, new() where TSocket : ISocketBase, new()
        {
        ...
        public T AddPlugin<T>(params object[] paramArray) where T : INetPluginBase, new()
        {
            Type type = typeof(T);
        ...
        public T GetPlugin<T>() where T : INetPluginBase, new()
        {
            Type type = typeof(T);
            if (plugins.ContainsKey(type))
                return (T)plugins[type];
