...
namespace FKGame
{
    public class UIBase : MonoBehaviour, UILifeCycleInterface
    {
        ...
        private Dictionary<string, GameObject> m_objects = null;
        ...
        private Dictionary<string, ReusingScrollRect> m_reusingScrollRects = new Dictionary<string, ReusingScrollRect>();
        ...
        private Dictionary<string, UGUIJoyStick> m_joySticks = new Dictionary<string, UGUIJoyStick>();
        private Dictionary<string, UGUIJoyStickBase> m_joySticks_ro = new Dictionary<string, UGUIJoyStickBase>();
        private Dictionary<string, LongPressAcceptor> m_longPressList = new Dictionary<string, LongPressAcceptor>();
        private Dictionary<string, DragAcceptor> m_dragList = new Dictionary<string, DragAcceptor>();
        ...
        private List<AnimData> animDataList = new List<AnimData>();

        public int UIID { get { return m_UIID; } }
        public string UIEventKey { get { return UIName + "@" + m_UIID; } }
        ...
        public UIModelShowData SetRawImageByModelShowCamera(string rawimageName, string modelName,
        ...
        {
            var model = CreateModelShow(modelName, layerName, orthographic, orthographicSize, backgroundColor, localPosition, 
                localScale, eulerAngles, texSize, nearClippingPlane, farClippingPlane);
        ...
        public UIModelShowData CreateModelShow(string modelName,
            string layerName = null,
            bool? orthographic = null,
            float? orthographicSize = null,
            Color? backgroundColor = null,
            ...
            float? farClippingPlane = null)
        {
            var model = UIModelShowTool.CreateModelData(modelName, layerName, orthographic, orthographicSize, backgroundColor, 
                localPosition, eulerAngles, localScale, texSize, nearClippingPlane, farClippingPlane);
            modelList.Add(model);
            return model;
        }

        public void RemoveModelShowCamera(UIModelShowData data)
        {
        ...
        // 新手引导获得动态创建Item  格式为：PetItem[0].Use（PetItem的Item上挂有UIBase脚本，
        // [0] 该名字的第几个Item，Use：拖到PetItem上的GameObject），
        // 当index小于0，则表示动态创建的列表List.Count-index(如-1：表示List.Count-1)
        ...
        public void CleanAnim()
        {
            ...
            {
                AnimManager.StopAnim(animDataList[i]);
            }
        }
