...
//------------------------------------------------------------------------
// 任何一个对象继承于 IJsonSerializable，实现两个接口
// 然后使用 Serialize/Deserialize 进行格式化
//------------------------------------------------------------------------
namespace FKGame
{
	public static class JsonSerializer 
    {
        ...
        public static List<T> Json2List<T>(string jsonData)
        {
            ...
            {
                List<object> listData = MiniJSON.Deserialize(jsonData) as List<object>;
                if (listData == null)
                    return datas;
        ...
        public static string List2Json<T>(List<T> datas)
        {
            ...
            return MiniJSON.Serialize(temp);
        }
        ...
        public static Dictionary<string, T> Json2Dictionary<T>(string jsonData)
        {
            ...
            if (!string.IsNullOrEmpty(jsonData))
            {
                Dictionary<string, object> listData = MiniJSON.Deserialize(jsonData) as Dictionary<string, object>;
                if (listData == null)
                {
                    return datas;
                }
        ...
        {
            //p.PropertyType.IsPrimitive IsPrimitive 表示是否为基元类型之一，则为 true；否则为 false。
            //基元类型是 Boolean、 Byte、 SByte、 Int16、 UInt16、 Int32、 UInt32、 Int64、 UInt64、 Char、 Double和 Single。
            if (t.IsPrimitive
                || t == typeof(decimal)
                || t == typeof(string)
                || t.IsEnum
