...
namespace FKGame
{
    public static class PersistentFileManager
    {
        ...
        {
            string data = "";
            string fullPath = PathTool.GetAbsolutePath(ResLoadLocation.Persistent, PathTool.GetRelativelyPath(c_directoryName, name, c_expandName));
        ...
        public static void SaveData(string name, string content)
        {
            ...
            ResourceIOTool.WriteStringByFile(
                PathTool.GetAbsolutePath(ResLoadLocation.Persistent,
                    PathTool.GetRelativelyPath(c_directoryName, name, c_expandName)), content);
    #if UNITY_EDITOR
